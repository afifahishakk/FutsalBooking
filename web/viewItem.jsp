<%-- 
    Document   : viewItem
    Created on : May 10, 2019, 10:46:38 AM
    Author     : USER
--%>
                    ...
                    <section id="content">
                        ...
                        <div class="table-wrapper">
                            <table border ="1">                  
                                ...
                                <tbody>
                                    <c:forEach items="${sessionScope.rqitemlist}" var="item" varStatus="loop">
                                        <tr>
                                            <td><c:out value="${loop.index + 1}" /> </td>
                                            <td><c:out value="${item.itemName}" /> </td>
                                            <td><c:out value="${item.itemType}" /> </td>
                                            <td><c:out value="${item.price}" /> </td>
                                            <td><c:out value="${item.bookDate}"/> </td>
                                            <td><c:out value="${item.bookingStat}"/> </td>
                                            <!--<td><c:out value="${item.payStatus}"/> </td>-->
                                            <c:if test="${item.bookingStat == 'approve'}">
                                                <c:if test="${item.payStatus == 'pending'}">
                                            ...
                                            <c:if test="${item.bookingStat == 'pending'}">
                                                <c:if test="${item.payStatus == 'pending'}">
                                            ...
                                            <c:if test="${item.bookingStat == 'approve'}">
                                                <c:if test="${item.payStatus == 'approve'}">
